...
for component in range(J):
    for i in range(Y.shape[0]):
        for j in range(Y.shape[1]):
            form_of_first_mini_sclm[0, 0, component] += \
                math.sqrt(Y[i, j]) * gamma[i, j, component]
            form_of_second_mini_sclm[0, 0, component] += \
                Y[i, j] * gamma[i, j, component]
...
first_sclm = np.sum(broadcast_3d_tile(first_mini_sclm, Y.shape[0], Y.shape[1], 1) * theta[:, :, 0, :],
                    axis=2)
second_sclm = np.sum(broadcast_3d_tile(second_mini_sclm, Y.shape[0], Y.shape[1], 1) * theta[:, :, 0, :],
                     axis=2)
